...
import { BOOKING, handleGetLocalStorage } from '@/utils/global';
...
const BankIdVerification = () => {
  ...
  const { setIsBankIdModalActive, isBankIdModalActive, selectedHospitalName } =
    useAppStore();
  ...
  React.useEffect(() => {
    if (verifyBankIdStatus.data?.isBankIdVerify) {
      ...
      if (flowType && flowType !== BOOKING && !selectedHospital) {
        toast.info(
      ...
      if (flowType && flowType === BOOKING) {
        ...
        if (
          ...
          createBooking.mutate({
            procedureId: selectedProcedureLocalstorage,
            gender: selectedGenderLocalstorage,
            patientPreferredStartDate: JSON.parse(
              selectedStartDateLocalstorage,
            ),
