...
import emptyIcon from '@/public/assets/images/empty_state_icon.png';
...
const ProcedureSelector = () => {
  ...
  const scrollToDiv = (index: number, procedureData: any) => {
    setSelectedDepartment(procedureData.id);
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center gap-2 pb-60">
      <div className="flex w-[50vw] flex-col items-center justify-center gap-2">
        ...
        <p className="mt-6 hidden text-center font-onsite text-base font-normal leading-7 text-dark-blue sm:block sm:text-xl">
      ...
      <div className="mb-9 mt-[40px] flex flex-col items-start gap-x-6 gap-y-3 sm:mb-32 sm:mt-[60px] sm:grid sm:grid-cols-3 sm:gap-6 xl:mb-4 xl:mt-[40px]">
            ...
            .map((procedureData, index) => {
              return (
                <FbtButton
                  ...
                  variant="outline"
                  className="flex !h-[62.95px] !w-[280px] cursor-pointer flex-row !items-center gap-x-2 gap-y-4 !rounded-2xl !border-none bg-primary-light-blue font-medium !text-extra-dark-blue hover:!bg-dark-blue hover:!text-white sm:my-0 sm:!h-[179.5px] sm:!w-[294px] sm:flex-col sm:!justify-center xl:!h-[120px]"
                  onClick={() => scrollToDiv(index, procedureData)}
                >
                  {/* {procedureAndIcon[
      ...
      {selectedDepartment && (
        <div className="mt-10 flex flex-col items-center justify-start gap-y-24">
          {departmentProcedureList.isSuccess &&
            departmentProcedureList.data &&
            departmentProcedureList.data?.data?.allCategoryWithProcedure &&
            Array.isArray(
            ...
            departmentProcedureList.data.data.allCategoryWithProcedure.map(
              (data) => {
                return (
                  selectedDepartment === data.id && (
                    <div
                      className="flex w-full flex-col items-start rounded-2xl bg-primary-light-blue p-6 sm:w-[660px]"
                      key={data.id}
                      ref={addToRefs}
                    >
                      <h3 className="text-4xl font-medium text-dark-blue">
                        {data.name[selectedLanguage]
                          ? data.name[selectedLanguage]
                          : data.name.en}
                      </h3>
                      <div className="my-6 h-px w-full bg-dark-blue" />
                      {data.procedures && data.procedures.length > 0 ? (
                        data.procedures.map((procedure) => {
                          return (
                            <div
                              key={procedure.id}
                              className="relative flex w-full items-center gap-y-6"
                            >
                              <input
                              ...
                              <label
                                ...
                                className="font-onsite text-base font-normal text-extra-dark-blue sm:cursor-pointer sm:text-2xl"
                              >
                        ...
                        <div className="flex w-full flex-col items-center justify-center gap-7">
                          ...
                          <p className="text-center font-onsite text-base font-normal text-dark-gray-text">
                            Currently, there are no procedures available in this
                            department. Please check back later or select a
                      ...
                      {data.subCategoryWithProcedures &&
                        ...
                        data.subCategoryWithProcedures.map(
                          (subCategoryData) => {
                            return (
                              <div
